...
export class RestaurantAPI extends API {
  ...
  public async fetchData(coord, radius, type): Promise<NearbySearchData> {
    ...
    const res = await fetch(
      `https://maps.googleapis.com/maps/api/place/nearbysearch/json?keyword=restaurant&location=${latitude}%2C${longitude}&radius=${radius}&type=${type}&key=${this.NEARBY_SEARCH_API_KEY}`
    )
